...
// state machine. When undefined, the ExternalEvent() pData argument must be created on
// the heap. The state machine will automatically delete the EventData pointer during
// state execution. When defined, clients must not heap allocate EventData with operator
// new. InternalEvent() used inside the state machine always heap allocates event data.
...
class GuardBase
{
 public:
  ...
  /// condition evaluates to TRUE the state action is executed. If FALSE, no state
...
template <class SM, void (SM::*Func)(void)> class ExitAction: public ExitBase
{
 public:
...
#define END_STATE_MAP                                                                    \
  ...
  ;                                                                                      \
  virtual const StateMapRow* GetStateMap() override { return &STATE_MAP[0]; }
